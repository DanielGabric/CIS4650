...
public class Semantic{
    ...
    public Semantic(DecList tree, boolean showTable){
        ...
        if(tree.size()>0){
            ...
            typOut.add(new VarType(Type.INT));
            Type inputType = new FunctionType(Type.INT, typIn);
            Type outputType = new FunctionType(Type.VOID, typOut);
    ...
    public void checkSemantics(SimpleVar tree, int level, boolean needInt){
        for(int i=level;i>=0;--i){
            if(symbol.get(i).containsKey(tree.name)){
                if(!(symbol.get(i).get(tree.name) instanceof VarType))
    ...
    public void checkSemantics(ArrayDec tree, int level, boolean needInt){
        ...
        Type t = new ArrayType(Type.INT, size);
    ...
    public void checkSemantics(ReturnExp tree, int level, boolean needInt){
        if(currentType == Type.INT){
            if(tree.exp == null){
                System.out.println("Returning nothing from a non-void function on line "+tree.pos);
            }else{
                checkSemantics(tree.exp, level, true);
            }
        }else{
            if(tree.exp != null){
                System.out.println("Returning something from a void function on line "+tree.pos);
            }
        }
    ...
    public void checkSemantics(FunctionDec fun, int level, boolean needInt){
        ...
        output += spacing(level)+"Function"+", "+fun.func+", "+typ+"\n";

        while(v != null){
            if(v.head instanceof ArrayDec){
                parameters.add(new ArrayType(Type.INT, 0));
            }else if(v.head instanceof SimpleDec){
                parameters.add(new VarType(Type.INT));
            }
            v = v.tail;
        }
        ...
        if(fun.result.typ == NameTy.INT){
            ...
            t = new FunctionType(Type.INT,parameters);
        }else{
            currentType = Type.VOID;
            t = new FunctionType(Type.VOID,parameters);
        }
